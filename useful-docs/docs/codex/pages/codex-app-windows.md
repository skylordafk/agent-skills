...
run in [Windows Subsystem for Linux (WSL)](https://developers.openai.com/codex/app/windows#windows-subsystem-for-linux-wsl).
...
The Codex app on Windows supports a native [Windows sandbox](https://developers.openai.com/codex/windows#windows-sandbox) when the agent runs in PowerShell, and uses Linux sandboxing when you run the agent in [Windows Subsystem for Linux (WSL)](https://developers.openai.com/codex/app/windows#windows-subsystem-for-linux-wsl). To apply sandbox protections in either mode, set sandbox permissions to **Default permissions** in the Composer before sending messages to Codex.
